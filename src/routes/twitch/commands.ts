...
routes.get("/followage/:streamer/:viewer", timing(), async (c) => {
  ...
  if (data.years && data.years > 0)
    parts.push(`${data.years} year${data.years === 1 ? "" : "s"}`);
  if (data.months && data.months > 0)
    parts.push(`${data.months} month${data.months === 1 ? "" : "s"}`);
  if (data.weeks && data.weeks > 0)
    parts.push(`${data.weeks} week${data.weeks === 1 ? "" : "s"}`);
  if (data.days && data.days > 0)
    parts.push(`${data.days} day${data.days === 1 ? "" : "s"}`);
  if (data.hours && data.hours > 0)
    parts.push(`${data.hours} hour${data.hours === 1 ? "" : "s"}`);
  if (data.minutes && data.minutes > 0)
    parts.push(`${data.minutes} minute${data.minutes === 1 ? "" : "s"}`);
  if (data.seconds && data.seconds > 0)
    parts.push(`${data.seconds} second${data.seconds === 1 ? "" : "s"}`);
...
routes.get("/chatter/:streamer", timing(), async (c) => {
  ...
  if (!chatters.success) {
    if (chatters.error instanceof Error) {
      ...
      if (
        ...
        chatters.error.message
          .toLowerCase()
          .includes("does not have moderator permissions")
      ) {
        return c.text(
          "The moderatorId provided in the request is not a moderator for the streamer. Please ensure the user is a moderator or remove the moderatorId from the request."
        );
  ...
  const chattingUsers =
    c.req.query("bots") === "true" || !KnownBots.success
      ? chatters.data
      : chatters.data.filter((user) => !KnownBots.data.has(user.user_login));
  ...
  safe(() => {
    c.env.FollowageApp.writeDataPoint({
      blobs: [
        ...
        users.streamer!.id,
        users.streamer!.login,
        `Chatters: ${chatters.data.length} (filtered: ${chattingUsers.length})`,
        "",
        moderatorId ?? "",
        (c.req.raw.cf?.colo as string) ?? "",
      ],
